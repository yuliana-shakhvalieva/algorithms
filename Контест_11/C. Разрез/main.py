...
class Graph:
    ...
    def add_edge(self, a, b, cap):
        ...
        self.edges.append(Edge(a, 0, cap, self.head[b]))
...
class Solution:
    def __init__(self):
        ...
        for i in range(len(self.graph.edges)):
            v_from = self.graph.edges[i ^ 1].to
            v_to = self.graph.edges[i].to
            if self.dist[v_from] != math.inf and self.dist[v_to] == math.inf:
                edges_min_cut.append(i // 2 + 1)

        print(len(edges_min_cut), max_flow)
        print(*sorted(edges_min_cut))
